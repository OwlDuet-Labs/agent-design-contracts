...
class ContractTreeRenderer:
    ...
    def render_file(self, qmd_file, output_pdf):
        ...
        cmd = ["quarto", "render", qmd_file.name, "--to", "pdf"]
    ...
    def render_tree(self, source_dir, output_dir):
        ...
        for i, qmd_file in enumerate(qmd_files, 1):
            ...
            if success:
                ...
                self.stats["failed"] += 1
                self.errors.append({"file": str(rel_path), "error": error})
...
def main():
    parser = argparse.ArgumentParser(description="Render .qmd contract files to PDF")
