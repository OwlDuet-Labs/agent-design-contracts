...
class ExecutionValidator:
    ...
    def validate_output_schema(
        self, result: Dict, expected_schema: Dict
    ) -> ValidationResult:
        ...
        if not isinstance(result.get("status"), str):
            return ValidationResult(valid=False, error="Status field must be string")

        if not isinstance(result.get("execution_time_ms"), (int, float)):
            return ValidationResult(
                valid=False, error="execution_time_ms field must be number"
            )
        ...
        if result.get("status") not in valid_statuses:
            return ValidationResult(
                valid=False, error=f"Status must be one of: {', '.join(valid_statuses)}"
            )
    ...
    def check_performance_constraints(self, metrics: Dict, constraints: Dict) -> bool:
        ...
        checks = [
            ...
            <= constraints.get("max_execution_time_ms", 30000),
    ...
    def validate_json_output(self, output_text: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(valid=False, error=f"Invalid JSON: {str(e)}")
    ...
    def validate_error_format(self, error_data: Dict) -> ValidationResult:
        ...
        for field in required_error_fields:
            if field not in error_data:
                return ValidationResult(
                    valid=False, error=f"Error missing required field: {field}"
                )
