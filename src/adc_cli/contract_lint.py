...
class ContractLinter:
        ...
        for line in lines:
            ...
            if "|" in line and not in_table:
                ...
                if len(fixed_lines) > 0 or "---" in line:
                    ...
                    fixed_lines.extend(["", "::: {.table-responsive}", ""])
                    in_table = True
    ...
    def lint_contract_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            if self.config.get("backup_originals", True) and not self.config.get(
                "dry_run", False
            ):
            ...
            if content != original_content:
                if self.config.get("auto_fix", True) and not self.config.get(
                    "dry_run", False
                ):
    ...
    def _validate_content(self, content: str) -> List[str]:
        ...
        if content.count("```{mermaid}") > 0:
            ...
            for block in mermaid_blocks:
                ...
                if node_count > 20:
                    issues.append(
                        f"Complex Mermaid diagram with {node_count} nodes - consider splitting"
                    )
    ...
    def run_contract_lint(self, base_dir: Optional[str] = None) -> Dict:
        ...
        for exclude_pattern in self.config.get("exclude_patterns", []):
            exclude_files = set(
                glob.glob(os.path.join(base_dir, exclude_pattern), recursive=True)
            )
...
def main():
    ...
    parser.add_argument(
        "--config", help="Path to configuration file (default: ~/.adcconfig.yaml)"
    )
