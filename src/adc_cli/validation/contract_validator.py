...
class ContractValidator:
    ...
    def validate_contract_file(self, contract_file: Path) -> Dict:
        ...
        contract_id = (
            contract_id_match.group(1) if contract_id_match else contract_file.stem
        )
        ...
        return {
            ...
            "file_path": str(contract_file),
            "markers_found": [
                f"ADC-IMPLEMENTS: <{marker['block_id']}>" for marker in markers_found
            ],
    ...
    def _generate_recommendations(self, contract_details: List[Dict]) -> List[str]:
        ...
        if total_missing > 0:
            recommendations.append(
                f"Implement {total_missing} missing contract blocks with ADC-IMPLEMENTS markers"
            )
    ...
    def validate_specific_contract(self, contract_id: str) -> ContractValidationResult:
        ...
        return ContractValidationResult(
            ...
            if result.get("implementation_status") == "implemented"
            else 0.5
            if result.get("implementation_status") == "partial"
            else 0.0,
            ...
            if result.get("missing_implementations")
            else [],
        )
